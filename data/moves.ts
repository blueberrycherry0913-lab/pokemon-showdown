...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	astonish: {
		...
		pp: 30,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, emotion: 1 },
		secondary: {
			chance: 10,
			volatileStatus: 'flinch',
		},
	...
	bittermalice: {
		...
		pp: 20,
		priority: 0,
		flags: { protect: 1, mirror: 1, metronome: 1, emotion: 1 },
		secondary: {
			chance: 25,
			boosts: {
				atk: -1,
	...
	celebrate: {
		...
		pp: 40,
		priority: 0,
		flags: { nosleeptalk: 1, noassist: 1, failcopycat: 1, failmimic: 1, failinstruct: 1, emotion: 1 },
	...
	chatter: {
		...
		priority: 0,
		flags: {
			protect: 1, mirror: 1, sound: 1, distance: 1,
	...
	confide: {
		...
		pp: 20,
		priority: 0,
		flags: { reflectable: 1, mirror: 1, sound: 1, metronome: 1 },
		boosts: {
			spa: -1,
		},
		target: "normal",
	...
	doomdesire: {
		...
		pp: 5,
		priority: 0,
		flags: { metronome: 1, futuremove: 1, emotion: 1 },
		onTry(source, target) {
	...
	grudge: {
		...
		onHit(target) {
			...
			if (lastMove) {
				const slot = target.moveSlots.find((s: { id: string }) => s.id === lastMove.id);
				if (slot) slot.pp = Math.max(0, slot.pp - 2);
			}
		},
		target: "normal",
		type: "Ghost",
	...
	healbell: {
		...
		pp: 5,
		priority: 0,
		flags: { snatch: 1, sound: 1, distance: 1, metronome: 1 },
		onHit(target, source) {
	...
	iceball: {
		...
		pp: 30,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, ball: 1 },
		secondary: {
			chance: 10,
			status: 'frb',
		},
	...
	landswrath: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, nonsky: 1, metronome: 1, emotion: 1 },
		target: "allAdjacentFoes",
		type: "Ground",
		zMove: { basePower: 185 },
		contestType: "Beautiful",
	...
	metalsound: {
		...
		pp: 40,
		priority: 0,
		flags: { protect: 1, reflectable: 1, mirror: 1, sound: 1, allyanim: 1, metronome: 1 },
		boosts: {
			spd: -2,
		},
		target: "normal",
	...
	moonblast: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, metronome: 1, heavyprojectile: 1 },
		secondary: {
			chance: 25,
			boosts: {
				spa: -1,
	...
	outrage: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, failinstruct: 1, emotion: 1 },
		self: {
	...
	playnice: {
		...
		pp: 20,
		priority: 0,
		flags: { reflectable: 1, mirror: 1, bypasssub: 1, metronome: 1, emotion: 1 },
		boosts: {
			atk: -1,
		},
		target: "normal",
	...
	powerwhip: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, vine: 1, metronome: 1 },
		target: "normal",
		type: "Grass",
		contestType: "Tough",
	},
	...
	ragefist: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, punch: 1, emotion: 1 },
		target: "normal",
		type: "Ghost",
	},
	ragepowder: {
	...
	ragingbull: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, emotion: 1 },
		onTryHit(pokemon) {
			// will shatter screens through sub, before you hit
			pokemon.side.removeSideCondition('reflect');
			pokemon.side.removeSideCondition('lightscreen');
	...
	roar: {
		...
		pp: 20,
		priority: -6,
		flags: { reflectable: 1, mirror: 1, sound: 1, allyanim: 1, metronome: 1, noassist: 1, failcopycat: 1 },
		forceSwitch: true,
		target: "normal",
		type: "Normal",
		zMove: { boost: { def: 1 } },
	...
	spite: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, reflectable: 1, mirror: 1, bypasssub: 1, metronome: 1, emotion: 1 },
		onHit(target) {
			let move: Move | ActiveMove | null = target.lastMove;
	...
	substitute: {
		...
		condition: {
			...
			onTryPrimaryHit(target, source, move) {
				if (target === source || move.flags['bypasssub'] || move.flags['sound'] || move.infiltrates) {
					return;
				}
	...
	takeheart: {
		...
		pp: 15,
		priority: 0,
		flags: { snatch: 1, metronome: 1, emotion: 1 },
		onHit(pokemon) {
	...
	uproar: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, sound: 1, metronome: 1, nosleeptalk: 1, failinstruct: 1, emotion: 1 },
		self: {
	...
	domainelectric: {
		...
		basePower: 0,
		category: "Status",
		name: "Domain: Electric",
		pp: 10,
		priority: 0,
		flags: {},
		pseudoWeather: 'electricdomain',
		target: "all",
		type: "Electric",
	},
	domainfairy: {
	...
	domainfighting: {
		...
		basePower: 0,
		category: "Status",
		name: "Domain: Fighting",
		pp: 10,
		priority: 0,
		flags: {},
		pseudoWeather: 'fightingdomain',
		target: "all",
		type: "Fighting",
	},
	domainfire: {
	...
	mindcontrolledtest: {
		...
		onTryHit(target) {
			...
			// Already Mind Controlled — cannot be applied again.
	...
	// §2 new weathers — test moves for playtesting. Each sets the corresponding weather
	// state. Weather-setting is meant to be rare (legendaries) in the real game; these are
	// playtest tools only. Clear Skies (TEST) sets the neutral named field state.
	rainbowtest: {
		num: -14,
		accuracy: true,
		basePower: 0,
	...
	fizzle: {
		...
		onHit(target, source) {
			if (this.randomChance(1, 10)) {
				this.trySetStatus(this.random(2) === 0 ? 'brn' : 'stun', target, source);
			}
		},
		target: "normal",
		type: "Dragon",
	...
	sparking: {
		...
		pp: 30,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, light: 1 },
		secondary: {
			chance: 25,
			status: 'stun',
		},
	...
	vinelash: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, vine: 1 },
		secondary: {
			chance: 50,
			volatileStatus: 'rooted',
		},
		target: "normal",
		type: "Grass",
	},
