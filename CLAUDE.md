...
### §7 Speed Tie Resolution (latest session)

Full implementation of §7: opposing move pairs that tie on speed resolve simultaneously. If both Pokémon KO each other when each is the last on their team, the result is a Draw. See §24 for the full technical reference.

**Server changes (`sim/battle.ts`, `sim/battle-queue.ts`):**
- `MoveAction` interface: new optional `speedTied?: boolean` flag
- 4 new class fields: `speedTiePartnerPending`, `speedTieDoubleKO`, `speedTieFirstMoveLogCheckpoint`, `speedTieFirstMoveLogs`
- `speedSort()` modified: when exactly 2 opposing move actions tie, tags both `speedTied = true` and orders them deterministically (side 0 first) instead of Fischer-Yates
- `case 'move'` block wrapped in braces for local variables; now handles first/second of pair detection, pre-move flag setting, log capture, and log reordering
- `faintMessages()` guard: returns immediately when `speedTiePartnerPending === true` (defers all faint processing until the paired move executes)
- Post-move code in `runAction`: after second move, clears `speedTiePartnerPending`, sets `speedTieDoubleKO` if both sides fainted, calls deferred `faintMessages()`
- `checkWin()`: uses `speedTieDoubleKO` to call `this.tie()` for the Draw outcome
- HTML announcement banner injected at start of speed-tied turn
- Log reordering: `[announcement][first |move|+simult][second |move|][first effects+simult][second effects]`

**Client changes (`src/battle.ts`, `src/battle-animations.ts`):**
- `speedtiecontact` animation in `BattleOtherAnims`: both sprites slide to midpoint (250ms decelerate), wisp burst fires at collision, both bounce back (350ms ballistic2Back)
- `speedTieAnimPending` field tracks whether next `|move|` is the second of a pair
- Modified `case 'move'` in `runMajor`: when `kwArgs.simult` or `speedTieAnimPending`, runs `speedtiecontact` instead of normal `animateMove`
- `run()` bypasses `maybeCloseMessagebar` when `speedTieAnimPending === true`, preventing the message bar from interrupting between the two paired moves
...
46. **`faintMessages()` is called from inside `runMove()` (via `trySpreadMoveHit`), not only from `runAction()`.** When implementing speed tie deferred faint logic, setting `speedTiePartnerPending = true` AFTER calling `runMove` for the first move is too late — the faint has already been processed mid-move. The flag must be set BEFORE calling `runMove` so every `faintMessages()` call from within `battle-actions.ts` sees the flag and defers. Lesson: when guarding code triggered from deep inside `runMove`, set the guard flag before `runMove`, not after it returns.

47. **The client message bar lifecycle causes `|move|` events to fire sequentially even when `[simult]` is appended.** When the first `|move|` event is processed, `this.log(args, kwArgs)` opens the message bar. The next event hits `maybeCloseMessagebar()` in `run()`, which detects a major→major section break and backs up `currentStep`, exits `nextStep()`, waits for the bar-close fade (~250ms), then calls `nextStep()` again — by which point the first Pokémon's animation is already running. Fix: bypass `maybeCloseMessagebar` when `speedTieAnimPending = true` so both `|move|` events are processed in the same `nextStep()` pass.

48. **`PokemonSprite.anim()` does NOT use `scene.timeOffset` — it chains jQuery animations directly on the element.** Animations on different elements are genuinely parallel in the browser's RAF loop with no extra synchronization needed. `showEffect()` DOES use `scene.timeOffset` (added to the effect's start time), so wisp/burst effects correctly time relative to when sprites arrive. When building animations that mix sprite movement and effects, sprite-to-sprite parallelism is automatic; only use `timeOffset` for effect timing.

49. **The `[simult]` kwarg already existed in Showdown for mega evolution minor events but was not wired up to `|move|` major events.** `nextStep()` handles `waitForAnimations === 'simult'` by resetting `timeOffset = 0` and continuing without waiting. Extending this to `|move|` required two client changes: (a) bypassing `maybeCloseMessagebar` between the two tied moves, and (b) setting `speedTieAnimPending = true` after the first `|move|` so the second knows to fire the clash animation instead of the normal lunge.
...
## 24. §7 Speed Tie Resolution technical reference

### Overview

§7 of the master reference states: when two opposing Pokémon choose moves that tie on speed, they act simultaneously rather than one going first via RNG. If both Pokémon KO each other (their last Pokémon), the result is a Draw.

### Architecture: two-flag deferred faint pattern

Four new class fields in `sim/battle.ts`:

```typescript
speedTiePartnerPending: boolean;            // true while waiting for the paired move to execute
speedTieDoubleKO: boolean;                  // true if both Pokémon fainted (for draw logic)
speedTieFirstMoveLogCheckpoint: number | null;  // log.length before first move's logs
speedTieFirstMoveLogs: string[] | null;         // captured first-move logs, for reordering
```

**Why `speedTiePartnerPending` must be set BEFORE `runMove`:** `faintMessages()` is called from inside `trySpreadMoveHit` in `battle-actions.ts` (mid-move, when the defending Pokémon drops to 0 HP), not only from `runAction()` after the move completes. Setting the flag after `runMove` returns is too late — the faint has already been processed. The flag must be up before any move execution begins.

**`speedTieDoubleKO` vs `speedTiePartnerPending`:** `speedTiePartnerPending` is cleared in the post-move block (after the second move executes) just before the deferred `faintMessages()` call. `speedTieDoubleKO` is set right before that call (if both sides have fainted), detected by `checkWin()` during faint processing, and cleared immediately after.

### `speedSort()` modification

When exactly 2 opposing move actions tie on speed, both are tagged `speedTied = true`, and they are ordered deterministically (side 0 first) rather than via Fischer-Yates shuffle.

### `case 'move'` block logic (server)
...
case 'move': {
    ...
    if (isFirstOfPair) {
        // inject HTML announcement banner + set deferred-faint flag
    ...
    if (isFirstOfPair) {
        // capture first move's logs for reordering
        this.speedTieFirstMoveLogs = this.log.splice(this.speedTieFirstMoveLogCheckpoint!);
    }

    if (isSecondOfPair && this.speedTieFirstMoveLogs) {
        // splice second move's logs, interleave with first using withSimult()
        // pattern: [first |move|+simult][second |move|][first effects+simult][second effects]
    }
    break;
}
```

**Post-`runAction` block** (after `clearActiveMove()`):
```typescript
...
### Log reordering for simultaneous visuals

The reordered output format:
```
[pre-move logs from first move (announcement, etc.)]
[first |move| line + |[simult]|]
[second |move| line]
[first move effects (damage, health, etc.) + |[simult]|]
[second move effects]
```

`sendUpdates()` only flushes `this.log` after the full turn loop, so mid-turn splicing is safe.

**`withSimult()` helper** tags effect lines with `|[simult]`, handling `|split|` protocol correctly (both player-visible and spectator-visible halves are tagged):
```typescript
const withSimult = (lines: string[]): string[] => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('|split|')) {
            out.push(line);
            if (++i < lines.length) out.push(lines[i] + '|[simult]');
            if (++i < lines.length) out.push(lines[i] ? lines[i] + '|[simult]' : lines[i]);
        } else {
...
### Speed tie announcement banner

When the first of a paired move executes, an HTML banner is injected via `this.add('html', ...)`:
```html
<div class="broadcast-blue" style="text-align:center;padding:6px 10px">
  <strong style="font-size:14pt">⚡ Speed Tie!</strong><br/>
  <strong>Poke1</strong> and <strong>Poke2</strong> are Speed Tied! They will act at the same time!
</div>
```

### `checkWin()` draw handling

```typescript
if (this.speedTieDoubleKO) {
    this.tie(); // Speed-tie double-KO: both last Pokémon KO'd each other → Draw (§7)
} else {
    this.win(faintData && this.gen > 4 ? faintData.target.side : null);
}
```

### Client-side: `speedtiecontact` animation

In `play.pokemonshowdown.com/src/battle-animations.ts`, added to `BattleOtherAnims`:
...
This animation is invoked via `scene.runOtherAnim('speedtiecontact' as ID, [poke, poke2])`. Both sprites animate in parallel (different DOM elements → jQuery `.animate()` is genuinely parallel in the browser's RAF loop). The defender's animation is mirrored by calling `runOtherAnim` again with arguments swapped.

### Client-side: `battle.ts` modifications
...
```
This prevents the first move's message bar from interrupting between the two paired moves. Both "X used Y!" announcements accumulate in the bar together; the bar closes only after both have been processed.

### Files changed

| File | Change |
|---|---|
| `sim/battle-queue.ts` | `speedTied?: boolean` added to `MoveAction` interface |
| `sim/battle.ts` | New fields, `speedSort()`, `case 'move'`, post-move faint deferred block, `checkWin()` draw branch |
| `play.pokemonshowdown.com/src/battle-animations.ts` | `speedtiecontact` in `BattleOtherAnims` |
| `play.pokemonshowdown.com/src/battle.ts` | `speedTieAnimPending` field, modified `case 'move'`, `maybeCloseMessagebar` bypass |
